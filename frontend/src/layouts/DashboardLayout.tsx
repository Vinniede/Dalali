...
export const DashboardLayout: React.FC<DashboardLayoutProps> = ({
  ...
  return (
    <div className="flex h-screen bg-gray-50">
      {/* Sidebar */}
      <aside className={`${sidebarOpen ? 'w-64' : 'w-20'} bg-gradient-to-b from-primary-700 to-primary-800 text-white transition-all duration-300 flex flex-col shadow-elevation z-40`}>
        {/* Logo Section */}
        ...
        <div className="border-t border-primary-600 p-4 flex flex-col gap-2 flex-shrink-0">
          <button
            ...
            className="w-full text-sm hover:bg-primary-600 py-2 rounded-lg transition-colors font-medium"
          >
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        {/* Top Header */}
        <header className="bg-white shadow-base border-b border-gray-200 px-6 sm:px-8 py-4 flex justify-between items-center sticky top-0 z-30">
          <div className="flex-1">
            <div className="flex items-center gap-2 mb-1">
              <h1 className="text-2xl sm:text-3xl font-bold text-gray-900">{title}</h1>
              {branchName && userRole === 'branch-admin' && (
                <span className="px-3 py-1 text-sm bg-secondary-100 text-secondary-800 rounded-full font-medium">
                  {branchName}
                </span>
              )}
            </div>
          ...
          <div className="relative">
            ...
            >
              ...
              <div className={`w-10 h-10 ${getRoleColor()} rounded-full flex items-center justify-center text-white font-bold shadow-md`}>
            ...
            {dropdownOpen && (
              <div className="absolute right-0 mt-2 w-48 bg-white rounded-lg shadow-lg border border-gray-200 py-1 z-50">
                <div className="px-4 py-3 border-b border-gray-200">
                  ...
                  <p className="text-sm text-gray-600">{userRole === 'super-admin' ? 'System Admin' : 'Branch Admin'}</p>
                </div>
                ...
                <div className="border-t border-gray-200 mt-1">
                  <button
                    onClick={handleLogout}
                    className="w-full text-left px-4 py-2 hover:bg-red-50 transition-colors text-red-600 font-medium"
                  >
                    🚪 Logout
                  </button>
                </div>
